...
import { approveUser, deleteParent } from "@/lib/admin/actions";
...
export function ParentsTable({ rows }: { rows: ParentRow[] }) {
  ...
  const [approvingId, startApprove] = useTransition();
        ...
        <>
              ...
              <tbody className="divide-y">
                {pageRows.map((parent) => (
                  <tr key={parent.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-5 py-4 text-gray-500">{parent.email}</td>
                    <td className="px-5 py-4">
                      {parent.approved ? (
                        ...
                        <button
                          ...
                          onClick={() =>
                            startApprove(async () => {
                              await approveUser(parent.user_id, parent.email, parent.full_name ?? "");
                            })
                          }
                          className="inline-flex items-center gap-1 text-xs bg-amber-100 text-amber-700 hover:bg-amber-200 px-2 py-0.5 rounded-full font-medium transition-colors disabled:opacity-50"
                        >
                          <UserCheck size={11} />
                          Aprobă
                        </button>
                    ...
                    <td className="px-5 py-4 text-gray-400 text-xs">
                      {parent.created_at ? new Date(parent.created_at).toLocaleDateString("ro-RO") : "—"}
                    </td>
                    <td className="px-5 py-4">
                      <div className="flex items-center gap-2">
                        <Button size="sm" asChild className="bg-blue-100 hover:bg-blue-200 text-blue-700 gap-1.5">
                          <Link href={`/admin/parents/${parent.user_id}/edit`}>
                            <Pencil size={14} />
                            Editează
                          </Link>
                        </Button>
          ...
          {pageCount > 1 && (
            <div className="flex items-center justify-between text-sm text-gray-500">
              <p>
                {filtered.length} utilizatori · pagina {page + 1} din {pageCount}
              </p>
