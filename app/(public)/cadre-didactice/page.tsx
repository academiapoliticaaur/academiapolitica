...
export const metadata: Metadata = { title: "Resurse Cadre Didactice — Ami & Moti" };
...
export default async function CadreDIdacticePage() {
  ...
  const [{ data: invatatorCourses }, { data: profesorCourses }] = await Promise.all([
    db.from("courses").select("id, title, slug, description, audience, age_group, estimated_duration, series_slug, series_order, series_title")
      .eq("status", "published").eq("audience", "invatator").is("deleted_at", null).order("order_index"),
    db.from("courses").select("id, title, slug, description, audience, age_group, estimated_duration, series_slug, series_order, series_title")
      .eq("status", "published").eq("audience", "profesor").is("deleted_at", null).order("order_index"),
  ]);

  const canSeeInvatator = isAdmin || (isApproved && accountType === "invatator");
  const canSeeProfesor = isAdmin || (isApproved && accountType === "profesor");
  ...
  const showInvatatorSection = isAdmin || accountType !== "profesor";
  const showProfesorSection = isAdmin || accountType !== "invatator";
  ...
  return (
    <div className="container mx-auto max-w-5xl px-4 py-12">
        ...
        <div className="mb-8 bg-indigo-50 border border-indigo-200 rounded-2xl p-6 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-indigo-700 text-sm mb-4">
              Creează un cont selectând tipul <strong>Învățător</strong> sau <strong>Profesor gimnaziu</strong> pentru a accesa resursele specifice rolului tău.
            </p>
      ...
      {/* Mesaj pentru părinți logați care nu sunt cadre didactice */}
      ...
      {/* Categoria Învățători — ascunsă pentru conturile de Profesor (nu îi privește) */}
      {showInvatatorSection && (
      <section className="mb-12">
        <div className="flex items-center gap-3 mb-5">
          <span className="text-2xl">🌈</span>
          <div>
            <h2 className="text-xl font-bold">Resurse Învățători — Clasele 0–4</h2>
            <p className="text-sm text-gray-500">Materiale pentru predarea AI literacy în ciclul primar</p>
          </div>
        </div>

        {!invatatorCourses?.length ? (
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {groupCourses(invatatorCourses as Course[]).map((group) =>
              group.type === "single" ? (
                <Link key={group.course.id} href={`/courses/${group.course.slug}`} className="bg-white rounded-xl border hover:border-indigo-300 hover:shadow-md transition-all p-5 group">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-xs text-indigo-600 font-medium bg-indigo-50 px-2 py-0.5 rounded-full">Învățător</span>
                  </div>
          ...
          <div className="bg-gray-50 rounded-xl border p-8 text-center">
            ...
            <p className="text-gray-500 text-sm">{invatatorCourses.length} cursuri disponibile pentru Învățători.</p>
            {!isLoggedIn && <p className="text-xs text-gray-400 mt-1">Creează un cont de Învățător pentru acces.</p>}
            {isLoggedIn && !isTeacher && <p className="text-xs text-gray-400 mt-1">Acces rezervat conturilor de tip Învățător.</p>}
          </div>
        )}
      </section>
      )}

      {/* Categoria Profesori — ascunsă pentru conturile de Învățător (nu îi privește) */}
      {showProfesorSection && (
      <section>
        <div className="flex items-center gap-3 mb-5">
          <span className="text-2xl">🚀</span>
