...
export const metadata: Metadata = { title: "Editează clasa — Academia Politica AUR" };
...
async function updateClass(classId: string, formData: FormData) {
  ...
  if (!name || !school_year || !access_code) {
    redirect(`/dashboard/grupuri/${classId}/edit?error=campuri_obligatorii`);
  }
  if (!/^[A-Z0-9]{4,12}$/.test(access_code)) {
    redirect(`/dashboard/grupuri/${classId}/edit?error=cod_invalid`);
  }
  ...
  if (error?.code === "23505") {
    redirect(`/dashboard/grupuri/${classId}/edit?error=cod_existent`);
  }

  redirect(`/dashboard/grupuri/${classId}`);
}
...
export default async function EditClassPage({
  ...
  return (
    <div className="max-w-xl">
      <div className="mb-6">
        <Link href={`/dashboard/grupuri/${id}`} className="flex items-center gap-2 text-sm text-gray-500 hover:text-gray-700 mb-4">
          <ArrowLeft size={16} />
          Înapoi la clasă
        </Link>
          ...
          <form action={updateClassAction} className="space-y-5">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <Button type="button" variant="outline" asChild>
                <Link href={`/dashboard/grupuri/${id}`}>Anulează</Link>
              </Button>
            </div>
          </form>
        </CardContent>
