...
export const metadata: Metadata = { title: "Detalii clasă — Academia Politica AUR" };
...
async function addStudent(classId: string, formData: FormData) {
  ...
  if (!display_name || !["0-4", "5-8"].includes(age_group))
    redirect(`/dashboard/grupuri/${classId}?tab=elevi&error=campuri_obligatorii`);

  const db = createAdminClient();
  const { data: cls } = await db.from("classes").select("id").eq("id", classId).eq("teacher_id", user.id).single();
  if (!cls) redirect("/dashboard/grupuri");
  ...
  const { data: newStudent } = await db.from("class_students")
    ...
    .select("id").single();
  redirect(`/dashboard/grupuri/${classId}?tab=elevi&newPin=${pin}&forStudent=${newStudent?.id ?? ""}`);
}
...
async function updateStudent(classId: string, studentId: string, formData: FormData) {
  ...
  const age_group = formData.get("age_group") as string;
  if (!display_name || !["0-4", "5-8"].includes(age_group))
    redirect(`/dashboard/grupuri/${classId}?tab=elevi&editStudent=${studentId}&error=campuri_obligatorii`);
  ...
  redirect(`/dashboard/grupuri/${classId}?tab=elevi`);
}
...
async function regenerateStudentPin(classId: string, studentId: string) {
  ...
  redirect(`/dashboard/grupuri/${classId}?tab=elevi&newPin=${newPin}&forStudent=${studentId}`);
}
...
async function assignCourse(classId: string, formData: FormData) {
  ...
  if (!course_id) redirect(`/dashboard/grupuri/${classId}?tab=cursuri`);
  ...
  redirect(`/dashboard/grupuri/${classId}?tab=cursuri`);
}
...
async function moveCourse(classId: string, courseId: string, direction: "up" | "down") {
  ...
  if (!rows || rows.length < 2) redirect(`/dashboard/grupuri/${classId}?tab=cursuri`);

  const idx = rows.findIndex((r) => r.course_id === courseId);
  if (idx < 0) redirect(`/dashboard/grupuri/${classId}?tab=cursuri`);

  const swapIdx = direction === "up" ? idx - 1 : idx + 1;
  if (swapIdx < 0 || swapIdx >= rows.length) redirect(`/dashboard/grupuri/${classId}?tab=cursuri`);
...
export default async function ClassDetailPage({
  ...
  const siteUrl = "https://academia-aur.ro";
  ...
  return (
    <div className="max-w-4xl">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <Button asChild variant="outline" size="sm" className="gap-1.5 text-indigo-600 border-indigo-200 hover:bg-indigo-50">
              <Link href={`/dashboard/grupuri/${id}/edit`}>
                <Pencil size={14} />
                Editează clasa
              </Link>
            </Button>
      ...
      <div className="flex gap-2 mb-6">
        <Link
          href={`/dashboard/grupuri/${id}?tab=elevi`}
        ...
        <Link
          href={`/dashboard/grupuri/${id}?tab=cursuri`}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            tab === "cursuri"
              ? "bg-indigo-600 text-white shadow"
              : "bg-white border-2 border-gray-200 text-gray-600 hover:border-indigo-300 hover:text-indigo-700"
        ...
        <Link
          href={`/dashboard/grupuri/${id}?tab=progres`}
          className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-semibold transition-all ${
            tab === "progres"
                      ...
                      <form action={updateStudent.bind(null, id, s.id)} className="flex flex-wrap gap-3 items-end">
                        ...
                        <div className="flex gap-1">
                          ...
                          <Button type="button" variant="outline" size="sm" className="h-8 px-3" asChild>
                            <Link href={`/dashboard/grupuri/${id}?tab=elevi`}><X size={13} /></Link>
                          </Button>
                        </div>
                      </form>
                    ) : (
                      ...
                      <div className="flex items-center justify-between gap-2 flex-wrap">
                        ...
                        <div className="flex items-center gap-2 flex-wrap justify-end">
                          ...
                          <Button variant="ghost" size="sm" className="text-gray-400 hover:text-indigo-600 h-8 w-8 p-0" asChild>
                            <Link href={`/dashboard/grupuri/${id}?tab=elevi&editStudent=${s.id}`}>
                              <Pencil size={14} />
                            </Link>
                          </Button>
                          {/* Delete */}
                    ...
                    <select id="course_id" name="course_id" required className="w-full h-9 px-3 rounded-md border border-input bg-background text-sm">
                      {availableCourses.map((c) => (
                        <option key={c.id} value={c.id}>
                          {c.title} ({c.audience === "children" ? "Elevi" : c.audience === "formator" ? "Formatori" : c.audience === "lector" ? "Profesori" : "Toți"})
                        </option>
                      ))}
                    </select>
                  </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-400">
                          {course.audience === "children" ? "Elevi" : course.audience === "formator" ? "Formatori" : course.audience === "lector" ? "Profesori" : "Toți"}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-1">
        ...
        <div className="space-y-4">
          {(students ?? []).length > 0 && assignedCoursesData.length > 0 && (
            <div className="flex justify-end">
              <a
                href={`/api/dashboard/grupuri/${id}/progress-csv`}
                download
