...
export default async function CourseLearningPage({ params }: PageProps) {
      ...
      if (newCert && user.email) {
        ...
        try {
          await sendDiplomaEmail({
            ...
            diplomaUrl: `${siteUrl}/cursant/${profileId}/certificate/${newCert.id}`,
  ...
  return (
    <div className="max-w-3xl mx-auto">
      <div className="mb-2">
        <Link href={`/cursant/${profileId}`} className="text-sm text-gray-400 hover:text-blue-500">
          ← Înapoi la cursuri
        </Link>
      </div>
      ...
      {isCourseComplete && (
        <div className="bg-gradient-to-r from-yellow-400 to-orange-400 text-white rounded-xl p-5 mb-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            {certificateId && (
              <Link
                href={`/cursant/${profileId}/certificate/${certificateId}`}
              ...
              {module.lessons?.map((lesson, lIdx) => {
                ...
                return (
                  <li key={lesson.id}>
                    {isApproved && hasSubscription ? (
                      <Link
                        href={`/cursant/${profileId}/course/${courseId}/lesson/${lesson.id}`}
