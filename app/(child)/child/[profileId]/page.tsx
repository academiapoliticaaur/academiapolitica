    ...
    <div className="max-w-4xl mx-auto space-y-5">
        ...
        <div className="bg-white rounded-2xl p-5 sm:p-6 border shadow-sm">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              <Link
                href={`/child/${profileId}/course/${activeCourse.id}`}
            ...
            >
              <Link href={nextLesson
                ? `/child/${profileId}/course/${activeCourse.id}/lesson/${nextLesson.id}`
                : `/child/${profileId}/course/${activeCourse.id}`
              }>
                Continuă →
              </Link>
            </Button>
        ...
        <div className="bg-gradient-to-r from-teal-50 to-blue-50 rounded-2xl p-5 sm:p-6 border border-teal-100">
          ...
          <div className="flex items-center gap-4">
            ...
            <Button asChild className="bg-teal-500 hover:bg-teal-600 text-white gap-1.5 shrink-0 hidden sm:flex">
              <Link href={`/child/${profileId}/course/${recommendedCourse.id}`}>
                Începe →
              </Link>
            </Button>
          </div>
      ...
      <div className="bg-white rounded-2xl border shadow-sm overflow-hidden">
        <div className="px-5 sm:px-6 py-4 border-b flex items-center justify-between">
          ...
          <Link href={`/child/${profileId}/transcript`} className="text-xs text-teal-600 hover:underline font-medium">
            📜 Diplome & transcript
          </Link>
        </div>
            ...
            <div className="space-y-3">
              {inProgressCourses.map((course) => {
                ...
                return (
                  <Link
                    key={course.id}
                    href={`/child/${profileId}/course/${course.id}`}
              ...
              {finishedCourses.map((course) => {
                ...
                return (
                  <div key={course.id} className="flex items-center gap-3 p-3 rounded-xl hover:bg-teal-50 transition-colors group">
                    ...
                    <div className="flex-1 min-w-0">
                      <Link href={`/child/${profileId}/course/${course.id}`} className="font-semibold text-sm text-gray-800 group-hover:text-teal-700 truncate block">{course.title}</Link>
                      <p className="text-xs text-teal-600 mt-0.5">✓ Finalizat · {cert?.total_points || 0} XP câștigați</p>
                    </div>
                    {cert?.id && (
                      <Link
                        href={`/child/${profileId}/certificate/${cert.id}`}
      ...
      <AmiMotiGuide
        variant={completedLessonsCount === 0 ? "ami" : streakDays >= 3 ? "moti" : "discovery"}
        message={
          completedLessonsCount === 0
            ? `Bun venit, ${childProfile.display_name}! Eu sunt Ami și te așteptam! Alege un curs de mai sus și pornești prima ta aventură în lumea cunoașterii. Hai, te provoc!`
            : streakDays >= 3
            ? `Felicitări, ${childProfile.display_name}! Eu, Moti, sunt impresionat — ${streakDays} zile consecutive de studiu! Menține ritmul și câștigă insigna de foc! 🔥`
            : `Eu, Moti, îți dau o misiune secretă: completează o lecție în fiecare zi și construiește un streak de zile consecutive! Cu cât e mai lung, cu atât mai tare ești! 💪`
        }
      />
    </div>
  );
